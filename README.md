...
| `git push --set-upstream <remoteName> <branchName>` | Upload changes to the remote (`-u` is shorthand for the option)   |
...
Corey and Tara are working on Project 3, and Corey is ready to push changes
...
corey in local-project-three on  coreys-feature-branch
$ git checkout master

# Fetch any changes from the remote
corey in local-project-three on  master
$ git fetch --all
...
corey in local-project-three on  master
$ git merge coreys-feature-branch

corey in local-project-three on  master
$ git push
```

After pushing the changes to the remote, Corey should communicate with the team
that the remote has been updated.

It is a good idea to fetch and pull changes as early and often as possible. Even
if Tara is still working on her own features, she can fetch and even merge the
...
tara in local-project-three on  taras-feature-branch
$ git fetch --all
...
# Pull down and merge any changes from the remote
tara in local-project-three on  master
$ git pull

# Since Tara is not ready to merge her changes to master, she can checkout the feature brach she was working on
tara in local-project-three on  master
$ git checkout taras-feature-branch
